 ...
 ***************************************************************
 *
 *          CM generic sockt management
 *
 * $Id: sockmisc.c,v 1.9 2009-10-07 16:01:31 guerin Exp $
 *
 ***************************************************************
 */
#include <sys/types.h>
...
setsock(int sock, int flags, int bufsize, char *sockname)
{
	...
	if (cm_debug_level & CM_DUMP_DBG_SOCK)
		syslog(LOG_DEBUG, "%s: %s socket flags: %d\n",
				__FUNCTION__, sockname, fcntl (sock, F_GETFL));
	...
	if (bufsize &&
		...
		if (sockname) {
			...
			else if (cm_debug_level & CM_DUMP_DBG_SOCK)
				syslog(LOG_DEBUG, "%s: %s socket send buffer size=%d\n",
						__FUNCTION__, sockname, optval);
		}
	}
	...
	if (bufsize &&
		...
		if (sockname) {
			if (getsockopt(sock, SOL_SOCKET, SO_RCVBUF, &optval, &optlen))
				...
				syslog(LOG_DEBUG, "%s: %s socket recv buffer size=%d\n", __FUNCTION__, sockname, optval);
		}
	}
}
